...
import { Search, Plus, MoreVertical, UserPlus, Shield, UserX, Mail, Phone, Calendar } from 'lucide-react';
...
const UserManagement = () => {
  ...
  const [filterRole, setFilterRole] = useState<'all' | 'admin' | 'manager' | 'support' | 'viewer'>('all');
  ...
  const getRoleBadge = (role: string) => {
    const roleConfig = {
      admin: { label: t('userManagement.roles.admin', 'Admin'), color: 'bg-purple-100 text-purple-800' },
      manager: { label: t('userManagement.roles.manager', 'Manager'), color: 'bg-blue-100 text-blue-800' },
      support: { label: t('userManagement.roles.support', 'Support'), color: 'bg-green-100 text-green-800' },
      viewer: { label: t('userManagement.roles.viewer', 'Viewer'), color: 'bg-gray-100 text-gray-800' },
    };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {t('userManagement.subtitle', 'Manage system users and their permissions')}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            >
              ...
              <option value="manager">{t('userManagement.roles.manager', 'Manager')}</option>
              <option value="support">{t('userManagement.roles.support', 'Support')}</option>
              <option value="viewer">{t('userManagement.roles.viewer', 'Viewer')}</option>
            </select>
            <select
              value={filterStatus}
              ...
              className="px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
            >
              <option value="all">{t('userManagement.allStatuses', 'All Statuses')}</option>
              <option value="active">{t('userManagement.status.active', 'Active')}</option>
              <option value="inactive">{t('userManagement.status.inactive', 'Inactive')}</option>
              <option value="suspended">{t('userManagement.status.suspended', 'Suspended')}</option>
            </select>
          </div>
        </div>
      </div>
              ...
              {filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <p className="text-gray-500">{t('userManagement.noUsers', 'No users found')}</p>
                  </td>
                </tr>
              ) : (
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500">
                            {t('userManagement.joined', 'Joined')} {new Date(user.createdAt).toLocaleDateString()}
                          </div>
                        </div>
                      </div>
                    </td>

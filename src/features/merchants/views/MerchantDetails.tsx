  ...
  if (error || !merchant) {
    return (
      <div className="text-center py-12">
        <p className="text-red-600">{t('merchants.errorLoadingMerchant', 'Error loading merchant details')}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        ...
        <span className={`px-3 py-1 rounded-full text-sm font-medium ${getVerificationStatusColor(merchant.verification_status)}`}>
          {t(`merchants.verificationStatus.${merchant.verification_status}`, merchant.verification_status)}
        </span>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-gray-900">{merchant.registration_method ? t(`merchants.registrationMethod.${merchant.registration_method}`, merchant.registration_method) : '-'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">
              {t('merchants.commercialRegisterNumber', 'Commercial Register Number')}
            </label>
            <p className="text-gray-900">{merchant.commercial_register_number || '-'}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">
              {t('merchants.businessesCount', 'Businesses Count')}
            </label>
            <p className="text-gray-900">{(merchant as any).businessesCount || '-'}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">
              {t('merchants.customersCount', 'Customers Count')}
            </label>
            <p className="text-gray-900">{(merchant as any).customersCountDistinct || '-'}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">
              {t('merchants.debtsCount', 'Debts Count')}
            </label>
            <p className="text-gray-900">{(merchant as any).debtsCount || '-'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-gray-900">{merchant.iban_verified ? t('common.yes', 'Yes') : t('common.no', 'No')}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">
          ...
          <div>
            ...
            <p className="text-gray-900">{(merchant as any).updated_at ? new Date((merchant as any).updated_at).toLocaleDateString() : '-'}</p>
          </div>
        </div>
      </div>
